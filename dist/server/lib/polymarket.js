...
exports.getPolymarketLinks = getPolymarketLinks;
...
async function fetchCityPolymarketUrl(city) {
    ...
    const match = events.find((event) => event.active &&
        !event.closed &&
        typeof event.slug === "string" &&
        typeof event.title === "string" &&
        event.title.toLowerCase().includes(`highest temperature in ${normalizedCity}`)) ??
        events.find((event) => event.active && !event.closed && typeof event.slug === "string");
    return match?.slug ? `${POLYMARKET_EVENT_BASE_URL}/${match.slug}` : null;
}
async function getPolymarketLinks() {
    if (cachedLinks && cachedLinks.expiresAt > Date.now()) {
        return cachedLinks.links;
    }
    const settled = await Promise.allSettled(airports_1.AIRPORTS.map(async (airport) => [
        airport.slug,
        await fetchCityPolymarketUrl(airport.city),
    ]));
    const links = Object.fromEntries(settled.map((result, index) => result.status === "fulfilled"
        ? result.value
        : [airports_1.AIRPORTS[index].slug, null]));
    cachedLinks = {
