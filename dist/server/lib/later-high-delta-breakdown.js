...
exports.createEmptyLaterHighDeltaBreakdown = createEmptyLaterHighDeltaBreakdown;
exports.buildLaterHighDeltaBreakdown = buildLaterHighDeltaBreakdown;
...
function createEmptyLaterHighDeltaSeries() {
    return {
        sampleDayCount: 0,
...
function createEmptyLaterHighDeltaBreakdown() {
    return {
        generatedAt: null,
        method: null,
...
function getClockMinutes(value, timezone) {
...
function parseObservedAtLocalMinutes(value) {
...
function getRoundedDisplayTemperature(value) {
...
function getComparableObservationValue(row, displayUnit) {
    if (displayUnit === "C") {
        if (typeof row.tempC === "number") {
            return row.tempC;
        }
        return typeof row.tempF === "number"
            ? (0, temperature_1.convertTemperature)(row.tempF, "F", "C")
            : null;
    }
    ...
    return typeof row.tempC === "number"
        ? (0, temperature_1.convertTemperature)(row.tempC, "C", "F")
        : null;
}
function buildMissingBreakdown(card, displayUnit, error) {
    const status = card.aviationWeatherCurrent.status === "error"
        ? "error"
...
function buildSeries(entries) {
    ...
    const samplesByDelta = new Map();
    let noHigherPeakDayCount = 0;
    const noHigherPeakSamples = [];
...
async function buildLaterHighDeltaBreakdown(card) {
    const displayUnit = (0, temperature_1.getAirportDisplayTemperatureUnit)(card.airport);
    const awNowObservedAt = card.aviationWeatherCurrent.observedAt;
    if (!awNowObservedAt) {
        return buildMissingBreakdown(card, displayUnit, card.aviationWeatherCurrent.error?.trim() || "AW now time is unavailable for later-jump history.");
    }
    ...
    const rows = await (0, db_1.getWundergroundObservationRows)({
    ...
    let basisDayCount = 0;
    const validDayResults = [];
    for (const [localDate, dayRows] of observationsByDate.entries()) {
        let basisObservation = null;
        let laterPeakObservation = null;
...
//# sourceMappingURL=later-high-delta-breakdown.js.map