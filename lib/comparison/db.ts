...
import type {
  ...
  StoredPolymarketDay,
  StoredWundergroundFutureHighHourlyStat,
  StoredWundergroundDaySummary,
  StoredWundergroundObservation,
} from "./types";

type SqlArgs = InValue[];
type ComparisonDateRangeParams = {
  startDate: string;
  endDate: string;
  citySlugs?: string[];
};
type ComparisonDayParams = {
  citySlug: string;
  localDate: string;
};
type ComparisonWindowParams = {
  citySlug: string;
  startDate: string;
  endDate: string;
};
...
declare global {
  var __polyweatherComparisonDbClient__: Client | undefined;
  var __polyweatherComparisonDbInitPromise__: Promise<void> | undefined;
}

const DEFAULT_DATABASE_URL = "file:./polyweather-web.db";

export function getComparisonDatabaseUrl() {
...
export function getComparisonDbClient() {
  if (!globalThis.__polyweatherComparisonDbClient__) {
    globalThis.__polyweatherComparisonDbClient__ = createClient({
...
async function execute(sql: string, args?: SqlArgs) {
  return getComparisonDbClient().execute(sql, args);
}

export async function ensureComparisonDb() {
...
function parseWinner(input: {
  ...
  const kind = String(input.kind) as ComparisonBucket["kind"];
...
function buildDateRangeQuery(table: string, params: ComparisonDateRangeParams, orderBy: string) {
...
function buildDateRangeFilter(
  params: ComparisonDateRangeParams,
  options?: {
    dateColumn?: string;
    cityColumn?: string;
  },
  ...
  const args: SqlArgs = [params.startDate, params.endDate];
...
async function selectDateRangeRows<T>(
  table: string,
  params: ComparisonDateRangeParams,
  orderBy: string,
...
async function selectOneRow<T>(
  table: string,
  params: ComparisonDayParams,
  mapRow: (row: Record<string, unknown>) => T,
) {
  await ensureComparisonDb();
  const result = await execute(
    `SELECT * FROM ${table} WHERE city_slug = ? AND local_date = ? LIMIT 1`,
    [params.citySlug, params.localDate],
  );
...
async function deleteWindowRows(table: string, params: ComparisonWindowParams) {
  await ensureComparisonDb();
  await execute(
    `DELETE FROM ${table} WHERE city_slug = ? AND local_date >= ? AND local_date <= ?`,
    [params.citySlug, params.startDate, params.endDate],
  );
}

async function selectDayRows<T>(
  table: string,
  params: ComparisonDayParams,
  orderBy: string,
  mapRow: (row: Record<string, unknown>) => T,
  ...
  await ensureComparisonDb();
  const result = await execute(
    `SELECT * FROM ${table} WHERE city_slug = ? AND local_date = ? ORDER BY ${orderBy}`,
    [params.citySlug, params.localDate],
  );
...
function mapWundergroundObservationRow(row: Record<string, unknown>): StoredWundergroundObservation {
  return {
    ...
    requestUrlImperial:
      row.request_url_imperial == null ? null : String(row.request_url_imperial),
    requestUrlMetric: row.request_url_metric == null ? null : String(row.request_url_metric),
    fetchedAt: String(row.fetched_at),
  };
}

function mapAviationObservationRow(row: Record<string, unknown>): StoredAviationObservation {
  const tempC = parseNullableNumber(row.temp_c);
  if (tempC == null) {
    throw new Error(
      `Stored AW observation is missing temp_c for ${String(row.city_slug)} ${String(row.observed_at_utc)}`,
    );
  }
...
function mapPolymarketDayRow(row: Record<string, unknown>): StoredPolymarketDay {
...
function mapWundergroundDaySummaryRow(row: Record<string, unknown>): StoredWundergroundDaySummary {
...
function mapWundergroundFutureHighHourlyStatRow(
...
function mapAviationDaySummaryRow(row: Record<string, unknown>): StoredAviationDaySummary {
  const pointCount = parseNullableNumber(row.point_count);
...
export async function getWundergroundObservationRows(params: ComparisonDateRangeParams) {
  return selectDateRangeRows(
    "wu_observations",
    params,
    "city_slug, local_date, observed_at_utc",
    mapWundergroundObservationRow,
  );
}

export async function getAviationObservationRows(params: ComparisonDateRangeParams) {
  return selectDateRangeRows(
    "aw_observations",
    params,
    "city_slug, local_date, observed_at_utc",
    mapAviationObservationRow,
  );
}

export async function getPolymarketDayRows(params: ComparisonDateRangeParams) {
  return selectDateRangeRows(
    "polymarket_days",
    params,
    "city_slug, local_date",
    mapPolymarketDayRow,
  );
}

export async function getResolvedPolymarketDayRows(params: ComparisonDateRangeParams) {
  await ensureComparisonDb();
  const { filters, args } = buildDateRangeFilter(params);
  const result = await execute(
    `SELECT * FROM polymarket_days WHERE ${filters.join(" AND ")} AND status = 'resolved' ORDER BY city_slug, local_date`,
    args,
  );

  return result.rows.map((row) => mapPolymarketDayRow(row as Record<string, unknown>));
}

export async function getLatestResolvedPolymarketDate(citySlug: string) {
  await ensureComparisonDb();
  const result = await execute(
    `SELECT MAX(local_date) AS latest_local_date
     FROM polymarket_days
     WHERE city_slug = ? AND status = 'resolved'`,
    [citySlug],
  );
...
export async function getWundergroundDaySummariesForResolvedPolymarket(
  params: ComparisonDateRangeParams,
) {
  await ensureComparisonDb();
  const { filters, args } = buildDateRangeFilter(params, {
  ...
  return result.rows.map((row) => mapWundergroundDaySummaryRow(row as Record<string, unknown>));
}

export async function getAviationDaySummariesForResolvedPolymarket(
  params: ComparisonDateRangeParams,
) {
  await ensureComparisonDb();
  const { filters, args } = buildDateRangeFilter(params, {
  ...
  return result.rows.map((row) => mapAviationDaySummaryRow(row as Record<string, unknown>));
}

export async function getWundergroundObservationDay(params: ComparisonDayParams) {
  return selectDayRows(
    "wu_observations",
    params,
    "observed_at_utc",
    mapWundergroundObservationRow,
...
export async function getAviationObservationDay(params: ComparisonDayParams) {
  return selectDayRows(
    "aw_observations",
    params,
    "observed_at_utc",
...
export async function getPolymarketDay(params: ComparisonDayParams) {
  return selectOneRow("polymarket_days", params, mapPolymarketDayRow);
}

export async function getWundergroundFutureHighHourlyStats(citySlug: string) {
  await ensureComparisonDb();
  const result = await execute(
    `
      SELECT *
  ...
  return result.rows.map((row) =>
    mapWundergroundFutureHighHourlyStatRow(row as Record<string, unknown>),
  );
}

export async function deleteWundergroundObservationWindow(params: ComparisonWindowParams) {
  return deleteWindowRows("wu_observations", params);
}

export async function deleteAviationObservationWindow(params: ComparisonWindowParams) {
  return deleteWindowRows("aw_observations", params);
}

export async function deletePolymarketDayWindow(params: ComparisonWindowParams) {
  return deleteWindowRows("polymarket_days", params);
}

export async function replaceWundergroundFutureHighHourlyStats(
  citySlug: string,
  records: StoredWundergroundFutureHighHourlyStat[],
) {
  await ensureComparisonDb();
  await execute(
    `
...
export async function upsertWundergroundObservations(records: StoredWundergroundObservation[]) {
  if (records.length === 0) {
    return 0;
  }
...
export async function upsertAviationObservations(records: StoredAviationObservation[]) {
  if (records.length === 0) {
    return 0;
  }
...
export async function upsertPolymarketDays(records: StoredPolymarketDay[]) {
  if (records.length === 0) {
    return 0;
  }
