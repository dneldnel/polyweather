...
import {
  ...
  replaceWundergroundFutureHighHourlyStats,
} from "./db";
import type { StoredWundergroundFutureHighHourlyStat } from "./types";

const MIN_ELIGIBLE_WU_OBSERVATIONS_PER_DAY = 12;
const FUTURE_HIGH_METHOD = "wu-running-max-hourly-v1";

type FutureHighAggregationRow = {
  city_slug: string;
  hour_bucket: number;
  probability: number;
  sample_count: number;
  future_higher_count: number;
...
export async function rebuildWundergroundFutureHighHourlyStatsForCity(citySlug: string) {
  ...
  const records = result.rows.map((row) => {
    const typedRow = row as unknown as FutureHighAggregationRow;
    ...
    return {
      ...
      generatedAt,
    } satisfies StoredWundergroundFutureHighHourlyStat;
  });

  await replaceWundergroundFutureHighHourlyStats(citySlug, records);
